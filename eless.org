# Time-stamp: <2018-02-22 13:33:09 kmodi>
...
- NOTE :: Above examples are tested to work in a *=bash=
          shell*. Specifically, examples like ~PAGER=eless man grep~
          might need to be adapted for the shell you are using, [[#example-eless-config-in-bash][and
          also the OS]].
...
man_page=$(echo "${first_line_piped_data}" \
               | perl -ne '/^([A-Za-z0-9-_]+\([0-9]+\))(?=\s+.*?\1$)/ and print $1' \
...
# For macOS systems, set PAGER to less and instead pipe the man output
# to eless.
alias eman '(setenv PAGER less; man \!* | eless)'
alias emang '(setenv PAGER less; man \!* | eless --gui)'
...
#   "PAGER=eless man ls", for example, would not work on macOS
# because of the way how man handles the stream on man pages on those
# systems. But with the below alias, "eman ls" will work. (Ref:
# https://github.com/kaushalmodi/eless/issues/27)
