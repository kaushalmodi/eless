# Time-stamp: <2017-05-03 11:59:09 kmodi>
...
script like =-D= and =--gui== get consumed here, and the one not known
to this script get passed to =emacs=. =getopt= does not support
ignoring undefined options. So the below basic approach of looping
through all the arguments ="$@"= is used.
#+BEGIN_SRC shell
for var in "$@"
do
    if [[ ${debug} -eq 1 ]]
...
# Local Variables:
# eval: (add-hook 'org-babel-post-tangle-hook #'delete-trailing-whitespace nil :local)
# End:
